...
const checkReviewComment = (req, res, next) => {
  if (!req.body.comment) {
    return res
      .status(400)
      .send(
        'Review comment is missing or incomplete in request body'
      );
  }
  next();
};
...
const checkReviewsBody = (req, res, next) => {
  ...
  if (!Array.isArray(reviews)) {
    return res
      .status(400)
      .send('Comments for reviews should be an array of objects');
  }

  const isOnlyCommentsFieldExists = reviews.every(
    (review) => Object.keys(review).length === 1 && review.comment
  );

  if (!isOnlyCommentsFieldExists) {
    return res
      .status(400)
      .send('Comment is empty or reviews contain unnecessary fields');
  }
  req.reviews = reviews;
